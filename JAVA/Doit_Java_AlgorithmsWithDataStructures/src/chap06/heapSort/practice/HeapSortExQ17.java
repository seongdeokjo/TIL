package chap06.heapSort.practice;

import java.util.Scanner;

public class HeapSortExQ17 {
	// downHeap 메서드가 호출될 때마다 배열의 값을 트리 형식으로 출력하는 프로그램을 작성
	...
	static void downHeap(int[] a, int left, int right) {
		...
		for (parent = left; parent < (right + 1) / 2; parent = child) {
			...
			int cr = cl + 1; // 오른쪽 자식
			child = (cr <= right && a[cr] > a[cl]) ? cr : cl; // 큰쪽
			if (temp >= a[child]) {
				break;
			}
			a[parent] = a[child];
	...
	static void heapSort(int[] a, int n) {
		for (int i = (n - 1) / 2; i >= 0; i--) { // a[i] ~ a[n - 1]를 힙으로 만든다.
			dispHeap(a, n);
			downHeap(a, i, n - 1);
		}
		for (int i = n - 1; i > 0; i--) {
			swap(a, 0, i); // 최대 요소와 아직 미정렬된 맨끝 요소를 교환
			dispHeap(a, n);
