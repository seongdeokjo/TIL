package chap09.linkedList.training;

import java.util.Comparator;
import java.util.Scanner;

public class AryLinkedListTester {
	// 연결 리스트 클래스 AryLinkedList<E> 사용 예
	...
	enum Menu {
		...
		NEXT("선택 노드를 하나 뒤쪽으로 이동"), PRINT_CRNT("선택 노드를 출력"), DUMP("모든 노드를 출력"), TERMINAE("종료");
	...
	static Menu SelectMenu() {
		...
		do {
			for (Menu m : Menu.values()) {
				...
				if ((m.ordinal() % 3) == 2 && m.ordinal() != Menu.TERMINAE.ordinal()) {
					System.out.println();
				}
			}
			System.out.print(" : ");
			key = stdIn.nextInt();
		} while (key < Menu.ADD_FIRST.ordinal() || key > Menu.TERMINAE.ordinal());
	...
	public static void main(String[] args) {
		...
		do {
			...
			case SEARCH_NO:
				...
				if (ptr == null) {
					System.out.println("그 번호의 데이터는 없다.");
				} else {
